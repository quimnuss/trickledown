...
func send_richmen_to_orbit(richman_data : RichmanData):
    ...
    # TODO use find
    for richman : Astronaut in richmen:
        if richman.richman_data == richman_data:
            richman_by_id = richman


    var portrait : OrbitPortrait = preload('res://ui/orbit_portrait.tscn').instantiate()
    portrait.richman_data = richman_data
